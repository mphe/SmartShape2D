...
@export var editor_debug = false : set = _set_editor_debug
@export var curve: Curve2D = null : set = _set_curve
@export var closed_shape = false : set = _set_close_shape
@export var auto_update_collider = false : set = _set_auto_update_collider
@export_range(1, 8) var tessellation_stages = 5 : set = _set_tessellation_stages
@export_range(1, 8) var tessellation_tolerence = 4 : set = _set_tolerence
@export var use_global_space = false : set = _set_use_global_space
@export_node_path var collision_polygon_node
@export_range(1, 512) var collision_bake_interval = 20
    ...
    for mesh in meshes:
        if (
            ...
            for m in mesh.meshes:
                # FIXME: pass mesh.normal_texture when support gets added
    ...
    if has_node(collision_polygon_node):
        ...
        if closed_shape:
            ...
            if not collision_quads.is_empty():
                ...
                points.push_back(
                    col_polygon.get_global_transform().xform_inv(
                        get_global_transform() * collision_quads[collision_quads.size() - 1].pt_d
