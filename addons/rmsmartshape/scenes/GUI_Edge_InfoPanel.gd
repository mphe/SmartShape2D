...
@export_node_path var p_btn_material_override
@export_node_path var p_ctr_override
@export_node_path var p_chk_render
@export_node_path var p_chk_weld
@export_node_path var p_int_index
@export_node_path var p_btn_edge_material
@export_node_path var p_btn_clear_edge_material
@export_node_path var p_lbl_edge_material

var indicies = [-1, -1] : set = set_indicies
var edge_material = null
var edge_material_selector = FileDialog.new()

signal on_set_material_override(enabled)
signal on_set_render(enabled)
signal on_set_weld(enabled)
signal on_set_z_index(value)
signal on_set_edge_material(value)
...
func set_edge_material(v: SS2D_Material_Edge, emit: bool = true):
    ...
    if emit:
        emit_signal("on_set_edge_material", v)
...
func _on_toggle_material_override(pressed: bool):
    ...
    emit_signal("on_set_material_override", pressed)


func _on_toggle_render(pressed: bool):
    emit_signal("on_set_render", pressed)


func _on_toggle_weld(pressed: bool):
    emit_signal("on_set_weld", pressed)


func _on_set_z_index(v: float):
    emit_signal("on_set_z_index", int(v))
