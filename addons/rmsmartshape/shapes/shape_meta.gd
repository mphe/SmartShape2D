...
func add_sibling(sibling: Node, legible_unique_name: bool = false):
...
func _update_cached_children():
    ...
    if treat_as_closed():
        can_edit = false
        if editor_debug:
            print ("META Shape3D contains Closed shapes, edit the meta shape using the child closed shape; DO NOT EDIT META DIRECTLY")
    else:
        can_edit = true
        if editor_debug:
            print ("META Shape3D contains no Closed shapes, can edit META shape directly")
